...
class WordnetSentenceSimilarity:
    ...
    def sentence_similarity(self, sentence1, sentence2):
        ...
        synsets1 = [
            self._get_synset(*pos_word)
            for pos_word in sentence1
            if self._get_synset(*pos_word)
        ]
        synsets2 = [
            self._get_synset(*pos_word)
            for pos_word in sentence2
            if self._get_synset(*pos_word)
        ]

        score = self._get_similarity_score(
            synsets1, synsets2
        ) + self._get_similarity_score(synsets2, synsets1)

...
class SentenceSimilaritySegmentizer:
    ...
    def generate_segments(self, youtube_video: TranscribedYoutubeVideo) -> Generator[Segment, None, None]:
        ...
        for idx, item in enumerate(youtube_video.transcript):
            ...
            if transcript_text.find(" ".join(end_sentence[-self.NUMBER_WORD_TO_FIND:])) != -1:
                body = youtube_video.transcript[start_index:idx+1]
                sentence_index += 1
                start_index = idx+1
                yield Segment(body)

    def _get_last_sentence_per_segment(self, youtube_video: TranscribedYoutubeVideo) -> List[str]:
        tokenized_text = sent_tokenize(self._get_text_string(youtube_video.transcript))
        end_sentences = []
        sentence_count = 0
        last_score = 0
        for i in range(len(tokenized_text)):
            if len(tokenized_text[i].split()) < self.MIN_SENTENCE_LENGTH or len(tokenized_text[i+1].split()) < self.MIN_SENTENCE_LENGTH:
                continue
            current_score = self.model.sentence_similarity(*tokenized_text[i: i+2])
            sentence_count += 1
            if current_score < self.SEGMENT_SPLIT_SCORE and sentence_count > self.MIN_SEGMENT_LENGTH:
                end_sentences.append(tokenized_text[i])
                sentence_count = 0
            elif current_score < self.SEGMENT_SPLIT_SCORE and sentence_count <= self.MIN_SEGMENT_LENGTH:
